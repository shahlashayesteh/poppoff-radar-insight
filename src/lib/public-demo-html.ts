...
const managerDemoSections: Record<string, DemoContent> = {
  [DEMO_MANAGER_PREFIX]: {
    ...
    bullets: [
      ...
      "Coaching Priorities",
      "Total Covers 812",
      "Estimated Uplift £1,420",
      "Server Viewed Stats 4 / 5",
    ],
  },
  [`${DEMO_MANAGER_PREFIX}/team`]: {
    ...
    bullets: [
      ...
      "Wine score by server",
      "Dessert score by server",
      "Bottled water score by server",
      "Scorecard engagement by server",
    ],
  },
  ...
  [`${DEMO_MANAGER_PREFIX}/menu`]: {
    ...
    bullets: [
      "Grilled Salmon with Sancerre",
      "Ribeye Steak with Malbec",
      "Chocolate Fondant with Espresso Martini",
      "Truffle Fries",
      "Sparkling Water",
    ],
  },
  [`${DEMO_MANAGER_PREFIX}/coaching`]: {
    ...
    bullets: [
      "Wine attachment during dinner shifts",
      "Bottled water consistency at lunch",
      "Dessert recommendation before bill presentation",
      "Pre-shift huddles",
      "1:1 coaching",
    ],
  },
...
function normalizePath(pathname: string) {
  ...
  return normalized || DEMO_MANAGER_PREFIX;
}
...
export function shouldReturnPublicDemoHtml(request: Request) {
...
export function publicDemoHtmlResponse(request: Request) {
  const { pathname } = new URL(request.url);
  ...
  const content = normalizedPath.startsWith(`${DEMO_MANAGER_PREFIX}/server/`)
    ...
    : managerDemoSections[normalizedPath] ?? managerDemoSections[DEMO_MANAGER_PREFIX];
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(content.title)} — PoppOff Manager Demo</title>
    <meta name="description" content="Public PoppOff manager demo page with readable manager dashboard content. No login required." />
    <link rel="canonical" href="https://poppoffstats.com${escapeHtml(normalizedPath)}" />
  ...
  <body>
    <main>
      ...
      <section class="grid" aria-label="Demo content">${content.bullets.map((item) => `<div class="card">${escapeHtml(item)}</div>`).join("")}</section>
...
export const publicDemoRouteHandler = (({ request, next }: { request: Request; next: () => unknown }) => {
  return shouldReturnPublicDemoHtml(request) ? publicDemoHtmlResponse(request) : next();
}) as any;