...
const wineByServer = servers.map((s) => ({
  name: s.name,
  score: s.wine === "green" ? 85 : s.wine === "amber" ? 60 : 40,
}));
const dessertByServer = servers.map((s) => ({
  name: s.name,
  score: s.desserts === "green" ? 88 : s.desserts === "amber" ? 62 : 38,
}));
const waterByServer = servers.map((s) => ({
  name: s.name,
  score: s.water === "green" ? 82 : s.water === "amber" ? 64 : 36,
}));
const engagementByServer = servers.map((s) => ({
  name: s.name,
  score: s.viewed ? (s.acknowledged ? 100 : 70) : 20,
}));
        ...
        <div className="mt-8 grid lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl bg-ink text-white p-6">
            <div className="text-xs uppercase tracking-widest text-white/60">
              Servers who have not viewed stats
            </div>
            <div className="mt-4 space-y-2">
              ...
              {servers.every((s) => s.viewed) && (
                <div className="text-sm text-white/60">Everyone has viewed this week.</div>
              )}
            </div>
          </div>
        </div>
