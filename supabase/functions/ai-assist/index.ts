...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "pair_chunk") {
      ...
      const { data: menus } = await admin.from("venue_menu").select("menu_text, parsed_items").eq("venue_id", venueId).order("uploaded_at", { ascending: false }).limit(10);
      const summary = (menus ?? []).map((m, i) => `--- Menu ${i + 1} ---\n${m.menu_text?.slice(0, 3500) || JSON.stringify(m.parsed_items)?.slice(0, 3500)}`).join("\n\n");
      const sys = "You are an expert sommelier, mixologist and pastry chef. For ONLY the food items listed by the user, produce pairings drawn from the venue's actual drinks/desserts menus. Categories to use: wine_bottle (wines sold by the bottle), wine_glass (wines available by the glass — look for 'by the glass', '125ml', '175ml', 'glass' markers in the wine list), cocktail, sake, beer, spirit, dessert. RULES: (1) For each food item × category, give UP TO 3 suggestions but ONLY if that many genuinely pair well — if only 2 truly suit, return just those 2; if none, omit. (2) When more than 3 pair well, pick the 3 MOST EXPENSIVE / premium options to maximise upsell. (3) Use ACTUAL menu items by name (with price if visible). (4) For BOTH wine_bottle AND wine_glass, the pair_with value MUST start with a style tag in square brackets: [White], [Red], [Rosé], or [Champagne] (use [Champagne] for any sparkling/prosecco/cava/crémant). Example: '[White] Sancerre 2022 £62'. (5) wine_bottle and wine_glass MUST be drawn from different price tiers when possible — never repeat the same wine in both. (6) Output one row per (food item, single pairing). Reply ONLY with JSON: {\"pairings\":[{\"item\":string,\"pair_with\":string,\"category\":\"wine_bottle\"|\"wine_glass\"|\"cocktail\"|\"sake\"|\"beer\"|\"spirit\"|\"dessert\"|\"other\",\"why\":string,\"priority\":\"High\"|\"Medium\"|\"Low\"}]}. Order within each (food, category) group most-to-least premium.";
