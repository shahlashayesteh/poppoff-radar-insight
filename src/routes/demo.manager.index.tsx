      ...
      <div className="flex-1 min-w-0">
        ...
        {sub && (
          <div
            ...
            style={{ color: subTone ?? "var(--brand-green)" }}
          >
            {sub}
          </div>
        )}
      ...
      <div className="px-8 py-7">
        ...
        <div className="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Stat
            ...
            value={`£${demoManagerKpis.avgSpc.toFixed(2)}`}
            sub="▲ +6.3% vs last week"
          />
          <Stat
            icon={TrendingUp}
            tone="var(--brand-green)"
            label="Modelled opportunity (week)"
          ...
          <Stat
            icon={Eye}
            tone="var(--brand-green)"
            label="Server Viewed Stats"
            value={`${demoManagerKpis.viewedCount} / ${demoManagerKpis.totalServers}`}
            sub={`${demoManagerKpis.viewedRatePct}%`}
            subTone="var(--muted-foreground)"
          />

          <Stat
            icon={Wine}
            tone="oklch(0.55 0.18 290)"
            label="Wine modelled opportunity"
            value={`£${demoManagerKpis.wineOpportunity}`}
            sub={`▲ +${demoCategoryOpportunities[0].deltaPct}% vs last week`}
          />
          <Stat
            icon={Cake}
            tone="var(--opportunity)"
            label="Dessert Performance"
            value={`+${demoManagerKpis.dessertDeltaPct}%`}
            sub="vs last week"
          />
          <Stat
            icon={Droplet}
            tone="oklch(0.65 0.15 240)"
            label="Bottled Water Progress"
            value={`+${demoManagerKpis.waterDeltaPct}%`}
            sub="vs last week"
          />
          <Stat
            icon={Target}
            tone="var(--opportunity)"
            label="Red Opportunities"
            value={String(demoManagerKpis.redOpportunities)}
            sub="▼ this week"
            subTone="var(--opportunity)"
          />
        </div>
        ...
        <div className="mt-6 rounded-2xl bg-white border border-border p-5">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Modelled uplift by category — sums to £{demoManagerKpis.uplift.toLocaleString()} this
            week.
          </p>
            ...
            {demoCategoryOpportunities.map((c) => (
              <div key={c.key} className="rounded-xl border border-border p-3">
                ...
                <div className="font-display text-xl font-extrabold mt-1">
                  £{c.uplift.toLocaleString()}
                </div>
                <div className="text-xs text-brand-green font-semibold mt-0.5">
                  +{c.deltaPct}% vs last week
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 grid lg:grid-cols-2 gap-4">
          <div className="rounded-2xl bg-white border border-border p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Margin earned per £ of labour. Full view in the LLS workspace.
                </p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <div className="font-display text-2xl font-extrabold">
                  {demoLabourLeveragePreview.scorePct}%
                </div>
                <div className="text-xs text-muted-foreground">
                  benchmark {demoLabourLeveragePreview.benchmarkPct}%
                </div>
              </div>
              <div>
                ...
                <div className="font-display text-2xl font-extrabold text-brand-green">
                  £{demoLabourLeveragePreview.monthlyOpportunity.toLocaleString()}
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Top gap</div>
                <div className="font-display text-base font-bold">
                  {demoLabourLeveragePreview.topGap.daypart}
                </div>
                <div className="text-xs text-opportunity font-semibold">
                  −£{demoLabourLeveragePreview.topGap.lostMargin.toLocaleString()}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white border border-border p-5">
            <div className="flex items-start gap-3">
              <div
                className="h-11 w-11 rounded-full grid place-items-center"
                style={{ background: "color-mix(in oklab, var(--brand-orange) 14%, white)" }}
              >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  How well the rota tracks forecasted demand this week.
                </p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-4 gap-3 text-sm">
              <div>
                ...
                <div className="font-display text-2xl font-extrabold">
                  {demoShiftMatchPreview.matchPct}%
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Over</div>
                <div className="font-display text-2xl font-extrabold text-opportunity">
                  {demoShiftMatchPreview.overstaffedShifts}
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Under</div>
                <div className="font-display text-2xl font-extrabold text-opportunity">
                  {demoShiftMatchPreview.understaffedShifts}
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Reshuffles</div>
                <div className="font-display text-2xl font-extrabold">
                  {demoShiftMatchPreview.recommendedReshuffles}
                </div>
              </div>
            </div>
          </div>
                  ...
                  <tr key={s.id} className="border-t border-border">
                    ...
                    {cats.map((c) => (
                      <td key={c.label} className="px-3 text-center">
                        <Dot s={(s as any)[c.key]} />
                      </td>
                    ))}
                    <td className="px-3 text-foreground/80">{s.weeklyFocus}</td>
                    <td
                      className={`px-3 text-center font-semibold ${s.viewed ? "text-brand-green" : "text-opportunity"}`}
                    >
                      {s.viewed ? "Yes" : "No"}
                    </td>
                    <td
                      className={`px-3 text-center font-semibold ${s.acknowledged ? "text-brand-green" : "text-opportunity"}`}
                    >
                      {s.acknowledged ? "Yes" : "No"}
                    </td>
                    <td className="px-3">
                      <Link
        ...
        <div className="mt-6 grid lg:grid-cols-3 gap-4">
          ...
          <div className="rounded-2xl bg-white border border-border p-5">
              ...
              {[
                {
                  i: Wine,
                  t: "Wine Attachment",
                  d: "Drive wine by the glass growth in punchlines.",
                  c: "oklch(0.55 0.18 290)",
                },
                {
                  i: Cake,
                  t: "Dessert Recommendation",
                  d: "Lead with dessert recommendation in punchlines.",
                  c: "var(--opportunity)",
                },
                {
                  i: Flame,
                  t: "Truffle Showcase",
                  d: "Recommend truffle across key dishes.",
                  c: "var(--brand-orange)",
                },
                {
                  i: Droplet,
                  t: "Bottled Water",
                  d: "Keep offering bottled water every shift.",
                  c: "oklch(0.65 0.15 240)",
                },
              ].map((p) => (
                <div key={p.t} className="flex gap-3">
                  <div
                    className="h-9 w-9 rounded-full grid place-items-center shrink-0"
                    style={{ background: `color-mix(in oklab, ${p.c} 14%, white)` }}
                  >
            ...
            <div
              className="mt-4 rounded-xl text-xs px-3 py-2 flex items-center gap-2"
              style={{
                background: "color-mix(in oklab, var(--brand-green) 10%, white)",
          ...
          <div className="rounded-2xl bg-white border border-border p-5">
            ...
            <p className="mt-3 text-sm text-foreground/80">
              You're on track! Keep up the coaching consistency and focus on wine attachment to
              unlock more uplift.
            </p>
            <svg viewBox="0 0 200 60" className="mt-4 w-full h-20">
              <path
                d="M0,55 C30,52 50,48 70,42 S110,30 130,22 S180,8 200,4"
                fill="none"
            ...
            <div className="mt-2 text-sm">
              <span className="text-brand-green font-bold">
                +£{demoManagerKpis.uplift.toLocaleString()}
              </span>{" "}
              <span className="text-muted-foreground">
                modelled opportunity this week (sum of per-server uplifts)
              </span>
            </div>
          </div>
              ...
              <div className="relative h-24 w-24">
                <svg viewBox="0 0 100 100" className="h-full w-full -rotate-90">
                  <circle
                    ...
                    r="40"
                    fill="none"
                    stroke="color-mix(in oklab, var(--brand-green) 14%, white)"
                    strokeWidth="10"
                  />
                  <circle
                    cx="50"
                    ...
                    strokeDashoffset={2 * Math.PI * 40 * (1 - demoManagerKpis.ackRatePct / 100)}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="absolute inset-0 grid place-items-center font-display text-xl font-extrabold">
                  {demoManagerKpis.ackRatePct}%
                </div>
              </div>
              <div className="text-sm">
                <div className="text-muted-foreground">Team has acknowledged this week's focus</div>
                <div className="mt-2 font-display text-xl font-extrabold text-brand-green">
                  {demoManagerKpis.ackCount}/{demoManagerKpis.totalServers} servers
                </div>
