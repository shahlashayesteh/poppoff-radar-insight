CREATE OR REPLACE FUNCTION public.csv_number(_row jsonb, _key text)
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  v_raw := trim(coalesce(_row ->> _key, ''));
  IF v_raw = '' OR lower(v_raw) IN ('null', 'n/a', 'na', '-') THEN
    RETURN 0;
  END IF;

  v_negative := v_raw ~ '^\(.*\)$' OR v_raw ~ '-\s*$';
...
REVOKE EXECUTE ON FUNCTION public.csv_number(jsonb, text) FROM anon;
